 ...
 *
 * These lists had drifted badly: both CLAUDE.md and API.md named 18 palettes
 * that do not exist (`blue`, `plasma`, `turbo`, `tableau10`, `set1`, …) while
 * omitting 13 that do. Nothing caught it because an unrecognised name resolves
 * to `undefined` and falls back to generated colours - a typo shows up as
 * "the colours look wrong", never as an error.
 */
...
/** Palette names mentioned in a documented list, keyed by category heading. */
function documentedNames(markdown: string, section: RegExp): Set<string> {
  const block = markdown.match(section);
...
describe('documented palette names match the implementation', () => {
  ...
  it('CLAUDE.md names exactly the palettes that exist', () => {
    const md = read('CLAUDE.md');
    const documented = documentedNames(
      md, /\*\*Built-in Palettes\*\*[\s\S]*?\n((?:- \*\*(?:Categorical|Sequential|Diverging)\*\*[^\n]*\n)+)/);

    expect(documented.size).toBeGreaterThan(0);
    const invented = [...documented].filter(n => !actual.has(n));
    const missing = [...actual].filter(n => !documented.has(n));

    expect(invented, `CLAUDE.md names palettes that do not exist: ${invented.join(', ')}`).toEqual([]);
    expect(missing, `CLAUDE.md omits real palettes: ${missing.join(', ')}`).toEqual([]);
  });
